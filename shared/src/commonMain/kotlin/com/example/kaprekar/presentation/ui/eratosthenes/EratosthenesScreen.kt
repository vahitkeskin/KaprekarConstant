...
fun EratosthenesScreen(
    ...
    Scaffold(
        topBar = {
            TopGradientAppBar(
                title = strings.topicEratosthenesTitle,
                state = state,
                onIntent = onIntent,
                showBackButton = true
            )
        ...
        LazyColumn(
            ...
            item {
                Card(
                    ...
                    Column(modifier = Modifier.padding(16.dp)) {
                        Text("🌍 C = 2πR (${strings.labelResult})", style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold)
                        Spacer(modifier = Modifier.height(8.dp))
                        Text(
                            text = "C = ${round(result.calculatedCircumferenceKm)} km",
                            fontSize = 26.sp,
                        ...
                        Text("R = ${round(result.calculatedRadiusKm)} km | Real: ${round(result.realCircumferenceKm)} km")
                        Text("%${round(result.accuracyPercentage)}", fontWeight = FontWeight.Bold, color = MaterialTheme.colorScheme.secondary)
                    }
                }
            }
            ...
            item {
                Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(16.dp)) {
                    Column(modifier = Modifier.padding(16.dp)) {
                        Text("☀️ ${strings.labelAngle} (θ)", fontWeight = FontWeight.Bold)
                        Spacer(modifier = Modifier.height(8.dp))
            ...
            item {
                Text("${strings.labelAngle} (θ): ${round(angleDeg.toDouble())}°", fontWeight = FontWeight.Bold)
                Slider(value = angleDeg, onValueChange = { angleDeg = it }, valueRange = 1f..15f)

                Text("${strings.labelDistance} (s): ${distKm.toInt()} km", fontWeight = FontWeight.Bold)
